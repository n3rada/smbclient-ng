...
if __name__ == "__main__":
    options = parseArgs()
    logger = Logger(
        no_colors=options.no_colors, debug=options.debug, logfile=options.logfile
    )
    ...
    for category in sorted(testCases.keys()):
        logger.info(
            "\x1b[1;48;2;170;170;170;30m├──[+] Category: %-84s\x1b[0m" % category
        )

        for subcategory in sorted(testCases[category].keys()):
            logger.info(
                "\x1b[1;48;2;200;200;200;30m├───┼──[+] Subcategory: %-77s\x1b[0m"
                % subcategory
            )
    ...
    logger.success("%-96s \x1b[0m" % "All done!")
    logger.info("Finished tests.")
    logger.info(
        "Tests PASSED: (%d/%d) %d%%"
        % (tests_passed, nb_testcases, (tests_passed / nb_testcases) * 100)
    )
    logger.info(
        "Tests FAILED: (%d/%d) %d%%"
        % (tests_failed, nb_testcases, (tests_failed / nb_testcases) * 100)
    )
