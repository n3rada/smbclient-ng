...
def _xdg_state_dir(app_name: str = "smbclientng") -> Path:
...
def setup_logging(
    level: str = "INFO", logfile: Optional[str] = None, no_colors: bool = False
):
    """
    ...
    Args:
        ...
        logfile: Optional path to a custom log file
    ...
    # Determine the format based on color preference
    if no_colors:
        # Simple format without colors
        console_format = "{time:YYYY-MM-DD HH:mm:ss.SSS!UTC} (UTC) [{level:7}] {message}\n{exception}"
    else:
        console_format = _format_message
    ...
    logger.add(
        ...
        colorize=not no_colors,
    )

    # --- File handler (rotating, UTC timestamps)
    if logfile is not None:
        # Handle existing logfile
        log_path = Path(logfile)
        if log_path.exists():
            k = 1
            while Path(f"{logfile}.{k}").exists():
                k += 1
            log_path = Path(f"{logfile}.{k}")
        ...
        logger.trace(f"Writing logs to logfile: '{log_path}'")
    else:
        # Use default XDG location
        log_dir = _xdg_state_dir()
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file = log_dir / "smbclientng.log"
